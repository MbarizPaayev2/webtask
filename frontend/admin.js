(function () {
  ...
  fetch("/api/admin/reports", { credentials: "same-origin" })
    ...
    .then(function (d) {
      ...
      if (statSum) {
        statSum.textContent =
          trans.length === 0
            ? "0"
            : sum.toLocaleString("az-Latn", { minimumFractionDigits: 2, maximumFractionDigits: 2 });
      }
      ...
      if (tbodyF) {
        ...
        if (flights.length === 0) {
          tbodyF.innerHTML =
            '<tr><td colspan="7" class="table-empty">Məlumat yoxdur — reys siyahısı boşdur.</td></tr>';
        } else {
      ...
      if (tbodyT) {
        ...
        if (trans.length === 0) {
          tbodyT.innerHTML =
            '<tr><td colspan="6" class="table-empty">Hələ əməliyyat qeydi yoxdur.</td></tr>';
        } else {
          trans.forEach(function (t) {
            var tr = document.createElement("tr");
            tr.innerHTML =
    ...
    .catch(function () {
      setBanner("Serverə qoşulmaq mümkün olmadı.");
    });
  ...
  if (tidGo && tidOut) {
    tidGo.addEventListener("click", function () {
      ...
      if (!id || id < 1) {
        tidOut.textContent = "Düzgün nömrə daxil edin.";
        return;
      }
      tidOut.textContent = "Yüklənir…";
